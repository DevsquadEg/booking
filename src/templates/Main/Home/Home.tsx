...
const deleteFromFavs = async (roomId: string) => {
  ...
  onToggleFavorite={(id) => {
    if (favoriteIds.includes(id)) { // ou check if that roomId is already in favList or not
      deleteFromFavs(id);
    } else {
      addToFavs(id);
    }
            ...
            {adsData.map((ad) => {
              ...
              return (
                <SwiperSlide key={room._id}>
                  <ImageCard
  roomId={room._id}
  image={room.images?.[0]?.replace("/upload/", "/upload/q_auto,f_auto/") || ""}
  title={""}
  price={room.price}
  isFirst={false}
  gridStyles={{ width: "100%", height: 250 }}
